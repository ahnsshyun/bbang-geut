...
- ✅ 전체 화면 UI 및 라우팅 구현 완료
- ✅ 주요 API 연동 완료 (로그인, 온보딩, 홈, 체크인, 기록, 병원 연결, 리포트)
- ✅ 한국어/일본어 다국어 적용 진행 중 (대부분 화면 완료)
- ⚠️ 일부 API 응답 필드가 `lang` 파라미터를 무시하고 한국어로 고정되어 오는 이슈 다수 발견 → 백엔드와 지속 확인 중
- ⚠️ 처방전 OCR 관련: 실제 이미지 인식이 아니라 "관리자가 사전 등록한 데이터를 조회"하는 방식이며, 테스트 계정에 데이터가 정확히 연결되어 있어야 정상 동작 (현재 일부 테스트 계정에서 `OCR_DRAFT_NOT_FOUND` 발생 중)
- ⚠️ PDF 저장, 병원 전달 등 일부 기능은 프론트 UI만 우선 구현되어 있고 실제 파일 생성/전송 로직은 TODO 상태
